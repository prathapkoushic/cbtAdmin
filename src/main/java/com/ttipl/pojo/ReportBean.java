package com.ttipl.pojo;

public class ReportBean {

	private String HTnumber;
	private String candidateName;
	private String isAnswer;
	private String session;
	private String marksPerQuestion;
	private String negativeMarksPerQuestion;
	private String exam_code;
	private String LocationName;
	private String TotalQuestions;
	private String TotalAnswered;
	private String TotalCorrect;
	private String negativeMarks;
	private String totalObtained;
	private int Rounded_marks;
	private String answer_option_id;
	private String No_of_questions;
	private String dob;
	private String community;
	private String responses;
	private String trade;
	private String post_category;
	private int exam_loc_id;
	
	

	public int getExam_loc_id() {
		return exam_loc_id;
	}

	public void setExam_loc_id(int exam_loc_id) {
		this.exam_loc_id = exam_loc_id;
	}

	public String getPost_category() {
		return post_category;
	}

	public void setPost_category(String post_category) {
		this.post_category = post_category;
	}

	public String getTrade() {
		return trade;
	}

	public void setTrade(String trade) {
		this.trade = trade;
	}

	public String getExam_code() {
		return exam_code;
	}

	public void setExam_code(String exam_code) {
		this.exam_code = exam_code;
	}

	public String getDob() {
		return dob;
	}

	public void setDob(String dob) {
		this.dob = dob;
	}

	public String getCommunity() {
		return community;
	}

	public void setCommunity(String community) {
		this.community = community;
	}

	public String getResponses() {
		return responses;
	}

	public void setResponses(String responses) {
		this.responses = responses;
	}

	public String getNo_of_questions() {
		return No_of_questions;
	}

	public void setNo_of_questions(String no_of_questions) {
		No_of_questions = no_of_questions;
	}

	public String getAnswer_option_id() {
		return answer_option_id;
	}

	public void setAnswer_option_id(String answer_option_id) {
		this.answer_option_id = answer_option_id;
	}

	public int getRounded_marks() {
		return Rounded_marks;
	}

	public void setRounded_marks(int rounded_marks) {
		Rounded_marks = rounded_marks;
	}

	public String getHTnumber() {
		return HTnumber;
	}

	public void setHTnumber(String hTnumber) {
		HTnumber = hTnumber;
	}

	public String getCandidateName() {
		return candidateName;
	}

	public void setCandidateName(String candidateName) {
		this.candidateName = candidateName;
	}

	public String getIsAnswer() {
		return isAnswer;
	}

	public void setIsAnswer(String isAnswer) {
		this.isAnswer = isAnswer;
	}

	public String getSession() {
		return session;
	}

	public void setSession(String session) {
		this.session = session;
	}

	public String getMarksPerQuestion() {
		return marksPerQuestion;
	}

	public void setMarksPerQuestion(String marksPerQuestion) {
		this.marksPerQuestion = marksPerQuestion;
	}

	public String getNegativeMarksPerQuestion() {
		return negativeMarksPerQuestion;
	}

	public void setNegativeMarksPerQuestion(String negativeMarksPerQuestion) {
		this.negativeMarksPerQuestion = negativeMarksPerQuestion;
	}

	public String getLocationName() {
		return LocationName;
	}

	public void setLocationName(String locationName) {
		LocationName = locationName;
	}

	public String getTotalQuestions() {
		return TotalQuestions;
	}

	public void setTotalQuestions(String totalQuestions) {
		TotalQuestions = totalQuestions;
	}

	public String getTotalAnswered() {
		return TotalAnswered;
	}

	public void setTotalAnswered(String totalAnswered) {
		TotalAnswered = totalAnswered;
	}

	public String getTotalCorrect() {
		return TotalCorrect;
	}

	public void setTotalCorrect(String totalCorrect) {
		TotalCorrect = totalCorrect;
	}

	public String getNegativeMarks() {
		return negativeMarks;
	}

	public void setNegativeMarks(String negativeMarks) {
		this.negativeMarks = negativeMarks;
	}

	public String getTotalObtained() {
		return totalObtained;
	}

	public void setTotalObtained(String totalObtained) {
		this.totalObtained = totalObtained;
	}

}
